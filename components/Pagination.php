...
class Pagination
{
    ...
    public function get()
    {
        ...
        if (!is_null($links)) {
            ...
            # Создаём ссылку "На первую"
                $links = $this->generateHtml(1, '<b>&lt;</b>') . $links;

            # Если текущая страница не первая
            if ($this->current_page < $this->amount)
            # Создаём ссылку "На последнюю"
                $links .= $this->generateHtml($this->amount, '<b>&gt;</b>');
        }
    ...
    {
        if ($this->total >= $this->limit && $this->total % $this->limit > 0){
            $result = round(($this->total / $this->limit)+1);
        } elseif ($this->total < $this->limit) {
            $result = 0;
        } elseif (($this->total / $this->limit) === 1) {
            $result = 0;
        } else {
            $result = round($this->total / $this->limit);
        }
        return $result;
    }
